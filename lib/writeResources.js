    ...
    return new Promise((resolve, reject) => {
      ...
      if (extension == '.webp' && options.decodeWebP) {
        if (options.encodeBasis) {
          ...
          decoder.write(`${os.tmpdir()}/image.png`, function(err) {
            exec(`basisu -mipmap ${os.tmpdir()}/image.png -output_path ${os.tmpdir()}`, (error, stdout, stderr) => {
              if (error) {
                console.log("Error", error);
                reject(error);
              }
              fs.readFile(`${os.tmpdir()}/image.basis`, (err, data) => {
                if (error) {
                  console.log("Error", error);
                  reject(error);
                }
