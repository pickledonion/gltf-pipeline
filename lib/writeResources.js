      ...
      if (extension == '.webp' && options.decodeWebP) {
        if (options.encodeBasis) {
          ...
          decoder.write(`${os.tmpdir()}/image-${uid}.png`, function(err) {
            exec(`basisu -mipmap ${os.tmpdir()}/image-${uid}.png -output_path ${os.tmpdir()}`, (error, stdout, stderr) => {
              if (error) {
                console.log("Error", error);
                reject(error);
              }
              fs.readFile(`${os.tmpdir()}/image-${uid}.basis`, (err, data) => {
                if (error) {
                  console.log("Error", error);
                  reject(error);
                }
                image.extras._pipeline.source = data
                fs.unlinkSync(`${os.tmpdir()}/image-${uid}.basis`)
                fs.unlinkSync(`${os.tmpdir()}/image-${uid}.png`)
                resolve(image)
              });
            })
          });
          ...
          gm(image.extras._pipeline.source, 'img.webp')
          .quality(options.jpegCompressionRatio || 80)
          .toBuffer('JPEG', (err, buffer) => {
            if (err) {
              reject(err)
            } else {
              image.extras._pipeline.source = buffer
              resolve(image)
            }
          }) 
        }
